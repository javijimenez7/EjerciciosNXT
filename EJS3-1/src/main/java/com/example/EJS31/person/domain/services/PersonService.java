...
import com.example.EJS31.person.infraestructure.repositories.PersonRepository;
...
public class PersonService implements PersonI {
    ...
    @Override
    public List<PersonOutputDTO> findByUser(String user) throws Exception{
        try{
            List<PersonOutputDTO> peopleByUser = personRepository.findByUser(user);
            return peopleByUser;
        } catch(Exception e){
            throw new Exception("No se ha podido encontrar el usuario");
        }
